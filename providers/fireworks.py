...
class FireworksProvider(BaseProvider):
    ...
    def __init__(self, config: Optional[ProviderConfig] = None):
        if config is None:
            ...
            config.provider_type = ProviderType.FIREWORKS
        ...
        **kwargs
        ...
        try:
            ...
            if hasattr(response.choices[0].message, 'tool_calls') and response.choices[0].message.tool_calls:
                tool_calls = self._parse_openai_tool_calls(response.choices[0].message.tool_calls)
            ...
            raise ProviderError(f"Fireworks generation failed: {str(e)}")
        ...
        try:
            ...
            async for chunk in stream:
                ...
                yield StreamChunk(
                    ...
                    tool_calls=self._parse_openai_tool_calls(delta.tool_calls) if hasattr(delta, 'tool_calls') and delta.tool_calls else None,
                )

        except Exception as e:
            raise ProviderError(f"Fireworks streaming failed: {str(e)}")
    ...
    async def tool_call(
        ...
        **kwargs
        ...
        """Execute tool calling with Fireworks AI."""
        temp = temperature if temperature is not None else self.config.temperature
        ...
        for iteration in range(max_iterations):
            try:
                ...
                raise ProviderError(f"Fireworks tool calling failed: {str(e)}")
    ...
    def _parse_openai_tool_calls(self, raw_tool_calls: Any) -> List[ToolCall]:
